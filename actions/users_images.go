...
func (v UsersImagesResource) Create(c buffalo.Context) error {
	...
	q := tx.Where("user_id = ?", currentUser(c).ID)
	exists, err := q.Exists("users_images")
	if err != nil {
		return errors.WithStack(err)
	}
	if exists {
		return c.Redirect(302, "/users/"+currentUser(c).ID.String())
	}
	...
	UsersImage.UserID = currentUser(c).ID
